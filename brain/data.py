...
def collect_oracle_dataset(n_episodes=200, seed=0, env_kwargs=None):
    ...
    scenes, poss, vels, goals, acts = [], [], [], [], []
    for _ in range(n_episodes):
        obs = env.reset()
        done = False
        while not done:
            a = env.oracle_action()
            scenes.append(obs["scene"]); poss.append(obs["pos"]); vels.append(obs["vel"])
            goals.append(obs["goal"]); acts.append(a)
            obs, _, done = env.step(a)
    to_t = lambda xs: torch.tensor(np.array(xs, dtype=np.float32))
    return {"scene": to_t(scenes), "pos": to_t(poss), "vel": to_t(vels),
            "goal": to_t(goals), "action": to_t(acts)}
